...
- 캔버스는 최대 `10x10` 정사각형 그리드 영역을 기준으로 동작한다.
- 초기 가시 영역은 `4x4`로 시작하고, 필요 시 `7x7`, `10x10` 단계로 확장된다.
- 사용자가 현재 가시 영역 경계 밖으로 노드를 드래그하면 다음 단계 영역으로 자동 확장된다.
- 그리드 영역 가이드는 드래그가 시작되면 옅은 색으로 표시되고, 드래그가 종료되면 숨긴다.
...
### Drop and Docking
- 드롭 자체는 어디서든 허용하되, 최종 착지(도킹)는 가장 가까운 그리드 내부 위치로 확정한다.
