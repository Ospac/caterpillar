...
/**
 * Render the interactive diagram canvas with controls for adding nodes, editing connections, and stage-aware grid behavior.
 *
 * The component manages node and edge state, shows a grid guide during node drags, clamps node positions to the active stage, advances the visible stage when nodes are dragged outside the current stage, and displays stage and occupancy information. Connections are created with a smoothstep edge and an arrow marker; edges can be removed by double-clicking.
 *
 * @returns The React element for the canvas including the Add Node control, stage/occupancy display, optional GridGuideOverlay, and the configured ReactFlow instance.
 */
